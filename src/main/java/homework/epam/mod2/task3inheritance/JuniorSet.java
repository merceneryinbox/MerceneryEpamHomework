...
public final class JuniorSet {
	...
	public void extendToMiddleSet() {
		...
		DrawingTool middlePen =
				new DrawingTool("middle firm", "Pilot pen", 156.22, "Middle type", "Three colors", true, true) {
					@Override
...
//    }
	...
	private List<Stationaries> sortMySetByCost() {
		...
		stationariesSet.addAll(new ArrayList(
				Arrays.asList(mergingSort(sortingCost)))); // fulfilling  cleared List by sorted elements of it
		return stationariesSet;
	}
	...
	private double[] merge(double[] left, double[] rigth) {
		...
		while (leftPointer < left.length) {
			for (; rigthPointer < rigth.length; ) {
				resultArInMerge[resultpointer] =
						left[leftPointer] < rigth[rigthPointer] ? left[leftPointer++] : rigth[rigthPointer++];
